...
void init_serial2(void) {
  ...
  */
  ...
  #define SERIAL_APBCLK (_APB1_CLOCK)

  #define INT_DIVIDER ((25UL * SERIAL_APBCLK) / (4 * BAUD))
...
void init_serial6(void) {
  ...
  */
  ...
  #define INT_DIVIDER_USART6 ((25UL * SERIAL_APB2CLK) / (4 * BAUD2))
  #define BAUD_H_USART6 ((INT_DIVIDER_USART6 / 100) << 4)
  #define FRACT_DIVIDER_USART6 (INT_DIVIDER_USART6 - (100 * (BAUD_H_USART6 >> 4)))
  #define BAUD_L_USART6 (((((FRACT_DIVIDER_USART6 * 16) + 50) / 100)) & 0X0F)

  USART6->BRR = BAUD_H_USART6 | BAUD_L_USART6;
...
uint8_t serial_rxchars(void) {
  return uartx_rxchars(USART2);
}
uint8_t serial_popchar(void) {
  return uartx_popchar(USART2);
}
...
uint8_t debug_popchar(void) {
  return uartx_popchar(USART6);
}
uint8_t debug_txchars(void) {
  return uartx_txchars(USART6);
}
