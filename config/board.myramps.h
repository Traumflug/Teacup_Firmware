...
#define DISPLAY  // if a display is attached to the printer

// ****** DO NOT USE DISPLAYBUS
// do not use display bus, since it is a consequence of the display being used;
// in actuality, the bus by itself is almost meaningless since each display has 
// its own particularities - delays, addressing, bus frequency for SPI/I2C etc. 
// As such, may be best to include these particulars in each display's driver 
// file
...
/////////////////////////////////////
/** \def DISPLAY_RS_PIN DISPLAY_E_PIN
    \def DISPLAY_D4_PIN DISPLAY_D5_PIN DISPLAY_D6_PIN DISPLAY_D7_PIN

  Pins necessary for the HD44780 driver. Taken into account with
  DISPLAY_TYPE_HD44780 defined, only.
*/
#define DISPLAY_RS_PIN           DIO59
#define DISPLAY_E_PIN            DIO63
#define DISPLAY_D4_PIN           DIO40
#define DISPLAY_D5_PIN           DIO64
#define DISPLAY_D6_PIN           DIO66
#define DISPLAY_D7_PIN           DIO65

/***************************************************************************\
*                                                                           *
* 7. ROTARY ENCODER SUPPORT                                                        *
*                                                                           *
\***************************************************************************/

/** 
  Update with your speciffic options. The below works for Rep Rap Discount
  Smart controller (4x20 HD44780 LCD + Rotary Encoder thingie)
  
  *** Note - the 128x64 graph screen is NOT supported (at least on AVR arch. 
		This is because we struggle to keep menus as "light" as possible on
		processing. While an ARM may have enough oomph to drive a graph display
		forcing additional bit processing on an AVR means less time to deal
		with printing.
		
  *** Note2 - we had to bump up the display update process from 1/s to 4/s 
		for text mode, since a 1/s display update is pretty laggy. Adding
		even more overhead dealing with a graph screen is a no-no for AVRs. 
		ARMs should cope just fine though.		
*/

#define ROTARY_ENCODER			// define this to have an interactive menu.
// without it, you only have a static System Status screen
// obviously, makes no sense to define it unless there is an actual rotary encoder
// (knob button thinghie) on your control pannel, so that you can use the knob
// to navigate menus. Most modern printers have this, with a few exceptions
// sticking to 5-button navigation.
// *** should you be faced with a 5-button navigation pannel, feel free to add button
// support to rot_enc.[ch]... in which case an appropriate file name would be 
// hid.[ch] or so :) 

#define ROT_EN1_PIN            DIO42
#define ROT_EN2_PIN            DIO44
#define BTN_ENC_PIN            DIO49

// here you select how "sensitive" the knob is. Actually, this is a built in characteristic
// of the encoder. Should your display cursor move too fast, define HALF/Q-speed
// if the cusor moves too slow (i.e. once every 2 detents) select a higher speed.
 
//#define ROT_ENC_SPEED_FULL
//#define ROT_ENC_SPEED_HALF
#define ROT_ENC_SPEED_QUARTER

#define ROT_ENC_REVERSE					// define this to reverse the button direction
