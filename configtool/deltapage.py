...
class DeltaPage(wx.Panel, Page):
  def __init__(self, parent, nb, idPg, font):
    ...
    self.geoKeys = ['DEFAULT_DELTA_DIAGONAL_ROD','DEFAULT_DELTA_RADIUS',
					'TOWER_X_ANGLE_DEG','TOWER_Y_ANGLE_DEG','TOWER_Z_ANGLE_DEG','DELTA_SEGMENTS_PER_SECOND','DELTA_SEGMENT_UM']
    ...
    self.labels = {'DEFAULT_DELTA_DIAGONAL_ROD':'Rod length:',
					...
					'DELTA_SEGMENTS_PER_SECOND':'Segment/s:',
					'DELTA_SEGMENT_UM':'Segment um:'}
  ...
  def insertValues(self, cfgValues):
    ...
    for k in self.textControls.keys():
      if k in cfgValues.keys():
        self.textControls[k].SetValue(cfgValues[k][0])
      else:
