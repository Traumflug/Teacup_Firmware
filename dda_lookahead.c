...
void dda_join_moves(DDA *prev, DDA *current) {
  ...
  uint32_t prev_F, prev_F_in_steps, prev_F_end_in_steps, prev_total_steps, prev_start_steps;
  ...
  if (prev->live == 0) {
    ...
    ATOMIC_START
      ...
      prev_start_steps = prev->start_steps;
    ATOMIC_END
    ...
    crossF_in_steps = MIN(crossF_in_steps, prev_total_steps + prev_start_steps);
    ...
    if (crossF_in_steps == prev_F_in_steps) {
      prev_rampup = prev_F_in_steps - prev_start_steps;
      prev_rampdown = 0;
    }
    else if (crossF_in_steps < prev_start_steps) {
      uint32_t extra, limit;

      prev_rampup = 0;
      prev_rampdown = prev_start_steps - crossF_in_steps;
      extra = (prev_total_steps - prev_rampdown) >> 1;
      limit = prev_F_in_steps - prev_start_steps;
      ...
      prev_rampup = crossF_in_steps - prev_start_steps;
      prev_rampdown = 0;
