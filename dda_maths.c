 ...
 * found here:http://stackoverflow.com/questions/1100090/looking-for-an-efficient-integer-square-root-algorithm-for-arm-thumb2
 */
uint32_t aSquareRoot32(uint32_t a_nInput)
{
    int32_t op  = a_nInput;
    int32_t res = 0;
    int32_t one = 1uL << 30; // The second-to-top bit is set: use 1u << 14 for uint16_t type; use 1uL<<30 for uint32_t type
