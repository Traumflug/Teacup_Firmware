...
static int32_t decfloat_to_int(decfloat *df, uint16_t multiplicand) {
  ...
  // e=1 means we've seen a decimal point but no digits after it, and
  // e=2 means we've seen a decimal point with one digit so it's too
  // high by one if not zero
  if (e)
    e--;
    ...
    if ((c >= 'A' && c <= 'Z') || c == '*' || (c == 10) || (c == 13)) {
      switch (last_field) {
        ...
        case 'E':
          ...
          break;
        case 'F':
          // just use raw integer, we need move distance and n_steps
          // to convert it to a useful value, so wait until we have
          // those to convert it
          if (next_target.option_inches)
          ...
          break;
        case 'S':
          // if this is temperature, multiply by 4 to convert to
          // quarter-degree units cosmetically this should be done in
          // the temperature section, but it takes less code, less
          // memory and loses no precision if we do it here instead
          if ((next_target.M == 104) || (next_target.M == 109) || (next_target.M == 140))
            next_target.S = decfloat_to_int(&read_digit, 4);
          // if this is heater PID stuff, multiply by PID_SCALE
          // because we divide by PID_SCALE later on
